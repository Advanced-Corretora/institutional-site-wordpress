...
// Detectar contexto usando função helper
$is_blog_context = advanced_corretora_is_blog_context();
$context_config = advanced_corretora_get_context_config();

// Adicionar classe ao body baseada no contexto
add_filter('body_class', function($classes) use ($is_blog_context) {
    if ($is_blog_context) {
        $classes[] = 'search-blog-context';
        $classes[] = 'blog-layout';
    } else {
        $classes[] = 'search-institutional-context';
    }
    return $classes;
});
...
<?php
// Carregar template específico baseado no contexto
if ($is_blog_context) {
    // Template para contexto do blog
    get_template_part('template-parts/search', 'blog');
    get_footer();
    return;
} else {
    // Template para contexto institucional (usa o template atual)
    // Continue com o template padrão abaixo
}
?>

<div class="container">
	<main id="primary" class="site-main search-main search-type-<?php echo esc_attr($search_type); ?> search-institutional-context">
